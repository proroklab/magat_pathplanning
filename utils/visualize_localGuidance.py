...
class Animation:
    def __init__(self, config):
        ...
        FOV_path_allagents = np.transpose(data_contents["FOV_Path"], (1, 0, 2,3))

        self.FOV_path = FOV_path_allagents[self.ID_agent,:,:,:]
        self.max_localPath = self.FOV_path.shape[1]
        # print(self.GSO.shape)
        # print(self.FOV_path.shape)
        # print(self.max_localPath)
        ...
        for d, i in zip(self.data_map["agents"], range(0, self.num_agents)):
            ...
            # if name == "agent{}".format(self.ID_agent):
                # set floating ID
            ...
            # if name == "agent{}".format(self.ID_agent):
            #     for id_path in range(self.max_localPath):
            #         #https://matplotlib.org/api/artist_api.html#module-matplotlib.lines
            #
            #         self.localPath[id_path] = Circle((d["start"][0], d["start"][1]),0.3, facecolor=self.list_color[i],
            #                                    edgecolor=self.list_color[i], alpha=0.5)
            #         self.localPath[id_path].original_face_color = self.list_color[i]
            #         self.patches.append(self.localPath[id_path])
    ...
    def animate_func(self, i):
        ...
        if i%10 == 0:
            ...
            # print(self.FOV_path[currentStep])
            # for id_path in range(self.max_localPath):
            #     # https://matplotlib.org/api/artist_api.html#module-matplotlib.lines
            #
            #     posX_FOV_Path = self.FOV_path[currentStep][id_path][0]
            #     posY_FOV_Path = self.FOV_path[currentStep][id_path][1]
            #     # print("({}/{} -- {}): ({},{})".format(currentStep, self.T, id_path, posX_FOV_Path,posY_FOV_Path))
            #
            #     self.localPath[id_path].center = (posX_FOV_Path,posY_FOV_Path)
        ...
        for agent_name in self.schedule["schedule"]:
            ...
            # if agent_name == "agent{}".format(self.ID_agent):
        ...
        for _, Path_item in self.localPath.items():
            Path_item.set_facecolor(Path_item.original_face_color)
