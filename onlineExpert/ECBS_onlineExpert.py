...
class ComputeECBSSolution:
    ...
    def runExpertSolver(self, id_case):
        ...
        try:
            ...
            map_setup = name_inputfile.split('input_')[-1].split('_IDMap')[0]
            id_input_map = name_inputfile.split('_IDMap')[-1].split('_IDCase')[0]
            id_input_case = name_inputfile.split('_IDCase')[-1].split('.yaml')[0]
            name_outputfile = self.dir_sol + 'output_{}_IDMap{}_IDCase{}_ECBS.yaml'.format(map_setup, id_input_map, id_input_case)
...
#               'map_w': 20,
#               'map_h': 20,
#               'failCases_dir': '/local/scratch/ql295/Data/MultiAgentDataset/experiments/dcpOEGAT_map20x20_rho1_10Agent/K3_HS0/1591834579/failure_cases/',
#               'exp_net': 'dcp'
#               }
