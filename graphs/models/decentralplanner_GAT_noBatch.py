...
class DecentralPlannerGATNet(nn.Module):
    def __init__(self, config):
        ...
        if self.config.use_dropout:
            dimActionMLP = 2
            numActionFeatures = [2 ** 7, numAction]
        else:
            dimActionMLP = 1
        ...
        for l in range(self.L):
            ...
            gfl.append(gml.GraphFilterBatchAttentional(self.F[l], self.F[l + 1], self.K[l], self.P[l], self.E, self.bias, concatenate=self.config.AttentionConcat,attentionMode=self.config.attentionMode))
