...
class StatisticAnalysis:
    ...
    def plot_sns_figure(self, metrics, df_metrics, title_end, text_legend, use_log_scale=True, save_fig=True):
        ...
        if metrics == "rate_ReachGoal":
            ax.legend(loc='lower left',ncol=2, borderaxespad=0.1,handleheight=0.1, columnspacing=0.2, labelspacing=0.05)
            start, end = ax.get_ylim()
            ax.set_ylim([-0.1, 1.05])
        elif metrics == "mean_deltaFT":
            # ## GNN
            ax.legend(loc='upper left', ncol=2, borderaxespad=0.1,handleheight=0.1, columnspacing=0.2, labelspacing=0.05)
            start, end = ax.get_ylim()
...
if __name__ == '__main__':
    ...
    ###############################################################
    DATA_FOLDER = '../MultiAgentDataset/Results_best/Statistics_deterministic/'
    epoch_text = "IROS"
