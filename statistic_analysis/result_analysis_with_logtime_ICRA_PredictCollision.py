...
class StatisticAnalysis:
                            ...
                            for action_policy in self.list_action_policy:
                                ...
                                if str(log_time) in filepath and action_policy in filepath:
                                    ...
                                    cleaned_data = {
                                        ...
                                        "num_CollisionPredicted": mat_data['num_CollisionPredicted'][0][0],
                                        "num_cases": len(mat_data['list_deltaFT'][0]),
                                        "rate_collision": np.divide(mat_data['num_CollisionPredicted'][0][0],
                                                                    len(mat_data['list_deltaFT'][0])),
                    ...
                    for metric in self.list_metrics:
                        ...
                        for index, num_agents in enumerate(self.label_num_agents):
                            ...
                            if len(scanned_list)>0:
                                if metric == "rate_collision":
                                    row_text += '{}\t'.format(np.round(scanned_list[0][metric], 3))
                                else:
    ...
    def print_error_bar_img(self, config, title):
        for metrics in self.list_metrics:
            ...
            elif metrics == "rate_collision":
                label_text_y = "Rate of Predicted Collision"
                # ax.set_ylabel(r'$\delta_{FT}$')
                # ax.set_ylabel(label_text_y)
                self.fig.set_size_inches(8, 7)
