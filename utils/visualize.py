...
class Animation:
    def __init__(self, config):
        ...
        self.fig = plt.figure(frameon=False, figsize=(20 * aspect, 20))
        ...
        for id_link in range(self.maxLink):
            ...
            self.commLink[name_link] = plt.Line2D((0, 0), (0, 0), linewidth=self.max_linewidth)
        ...
        for d, i in zip(self.data_map["agents"], range(0, self.num_agents)):
            ...
            if self.num_agents >self.max_numAgent  and name == "agent{}".format(self.ID_agent):
                self.agent_names[name] = self.ax.text(d["start"][0], d["start"][1], name.replace('agent', ''), fontsize=48)
