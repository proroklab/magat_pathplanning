...
class DecentralPlannerGATNet(nn.Module):
    def __init__(self, config):
        ...
        if self.config.AttentionConcat:
            numActionFeatures = [self.F[-1]*self.config.nAttentionHeads + numCompressFeatures[-1]] + numActionFeatures
        else:
            numActionFeatures = [self.F[-1]+numCompressFeatures[-1]] + numActionFeatures
        actionsfc = []
    ...
    def obtainAttentionGSO(self, attentionGSO):
        self.aij = attentionGSO
    ...
    def forward(self, inputTensor):
        ...
        # B x N x F (input_feature) ->  B x F (input_feature) x N
        ...
        if self.config.return_attentionGSO:
            attention_S = []
            for l in range(self.L):
                attention_S.append(self.GFL[l].returnAttentionGSO())
            attention_S_np = np.mean(np.asarray(attention_S), axis=0)
            self.obtainAttentionGSO(attention_S_np)


        sharedFeature_stack =sharedFeature.permute([0,2,1]).to(self.config.device).reshape(B*N,num_G)

        # Skip connection
        sharedFeature_skipConCate = torch.cat((compressfeature, sharedFeature_stack), dim=1)
