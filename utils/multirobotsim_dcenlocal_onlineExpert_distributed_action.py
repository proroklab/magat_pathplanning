...
class multiRobotSim:
    ...
    def getGSO(self, step):
        ...
        GSO, communicationRadius, graphConnected = self.getAdjacencyMatrix(step, store_PosAgents,self.communicationRadius)
    ...
    def move(self, actionVec, currentstep):
        ...
        # if not allReachGoal and currentstep < self.maxstep:
            for id_agent in range(self.config.num_agents):
                ...
                # actionVec_current_softmax = self.fun_Softmax(actionVec[id_agent])
                # actionKey_predict_softmax = torch.max(actionVec_current_softmax, 1)[1]
                #
                # print(actionVec[id_agent])
                # print(actionVec_current_softmax)
                #
                # print(actionKey_predict_softmax)
                actionKey_predict = self.convectToActionKey(actionVec[id_agent])
                if self.config.batch_numAgent:
                    # Fix data type when using batch
                    actionKey_predict = torch.ones(1).long() * actionKey_predict
                # print(actionKey_predict.dtype)
    ...
    def save_success_cases(self, mode):
        ...
        inputfile_name = os.path.join(self.config.result_AnimeDemo_dir_input,
                                      'input_map{:02d}x{:02d}_IDMap{:05d}_IDCase{:05d}.yaml'.format(self.size_map[0], self.size_map[1],self.ID_MAP, self.ID_case))
        print(inputfile_name)

        outputfile_name = os.path.join(self.config.result_AnimeDemo_dir_predict_success,
                                       'predict_map{:02d}x{:02d}_IDMap{:05d}_IDCase{:05d}.yaml'.format(self.size_map[0], self.size_map[1],self.ID_MAP, self.ID_case))
        if mode == 'success':
            checkSuccess = 1
        else:
            checkSuccess = 0
        ...
        targetfile_name = os.path.join(self.config.result_AnimeDemo_dir_target,
                                       'expert_map{:02d}x{:02d}_IDMap{:05d}_IDCase{:05d}.yaml'.format(self.size_map[0], self.size_map[1],self.ID_MAP, self.ID_case))
    ...
    def save_failure_cases(self):
        ...
        inputfile_name = os.path.join(self.failureCases_input, 'input_map{:02d}x{:02d}_IDMap{:05d}_IDCase{:05d}.yaml'.format(self.size_map[0], self.size_map[1],self.ID_MAP, self.ID_case))
