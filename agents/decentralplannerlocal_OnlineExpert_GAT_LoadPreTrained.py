'''
Loading pre-trained paramters from models trained by decentralplannerlocal_OnlineExpert_GAT.py
Warnings: As fine-tuning models were not used in our paper, these are only concept-proof codes.
        These codes are already out-of-date.
        For any formal use, please sync this file with decentralplannerlocal_OnlineExpert_GAT.py
        and debug.
'''
import shutil
...
class DecentralPlannerAgentLocalWithOnlineExpertGATLoadOE(BaseAgent):
    ...
    def __init__(self, config):
        ...
        self.start_with_freeze_SomeLayer = False
        ...
        if self.config.train_TL:
            if self.config.exp_net == 'dcpOEGAT' and self.config.exp_net_load != 'dcpOEGAT':
                self.load_pretrained_checkpoint_for_Attention(self.config.test_epoch, lastest=self.config.lastest_epoch,
                                                              best=self.config.best_epoch)
            else:
                self.load_pretrained_checkpoint(self.config.test_epoch, lastest=self.config.lastest_epoch,
                                                best=self.config.best_epoch)
        elif self.config.test_general:
            self.load_pretrained_checkpoint(self.config.test_epoch, lastest=self.config.lastest_epoch,
                                            best=self.config.best_epoch)
        else:
    ...
    def load_pretrained_checkpoint_for_Attention(self, epoch, lastest=True, best=False):
        """
        Latest checkpoint loader
        :param file_name: name of the checkpoint file
        :return:
        ...
        try:
            if self.config.train_TL:
                ...
                self.current_epoch = 0#checkpoint['epoch']

                self.current_iteration = 0 #checkpoint['iteration']

                load_model = checkpoint['state_dict']
                init_model = self.model.state_dict()
                list_loaded_param_name = ['*Conv*', '*compressMLP*']
                # param_name_GFL = '*GFL*'

                # list_loaded_param_name = ['*Conv*', '*compressMLP*', '*GFL*']
                # param_name_GFL = '*GFL*'

                param_name_GFL = '*GFL*'
                param_name_action = '*actionsMLP*'

                assert param_name_GFL != '', 'you must specified the name of the parameters to be re-trained'
                for model_param_name, model_param_value in self.model.named_parameters():
                    # print("---All layers -- \n", model_param_name)

                    if any(fnmatch(model_param_name, load_param_name) for load_param_name in list_loaded_param_name):
                        print("--- Loaded weight from pretrained trained layers -- \n\t{}\n".format(model_param_name))
                        model_param_value.requires_grad = False
                        # model_param_value.requires_grad = True
                        # print(load_model[model_param_name])
                        init_model.update({model_param_name: load_model[model_param_name]})
                    elif fnmatch(model_param_name, param_name_GFL):
                        # print("--- Loaded weight from pretrained trained layers -- \n")
                        # print("\t{}\t{}\n".format(model_param_name, model_param_value.shape))

                        # print("\t{}\t{}\n".format('GFL.0.weight	', load_model['GFL.0.weight'].shape))
                        load_model_GFL_weight = load_model['GFL.0.weight'].permute(2, 1, 0, 3)
                        load_model_GFL_weight_nHead = load_model_GFL_weight.repeat(self.config.nAttentionHeads, 1, 1, 1, 1)

                        # print("\t{}\t{}\t\{}\n".format('GFL.0.weight', load_model_GFL_weight.shape,load_model_GFL_weight_nHead.shape))

                        # print("\t{}\t{} \n {}\n \t\{}\n {}\n".format('GFL.0.weight	', load_model_GFL_weight.shape,load_model_GFL_weight, load_model_GFL_weight_nHead.shape,load_model_GFL_weight_nHead))

                        model_param_value.requires_grad = True
                        # init_model.update({model_param_name: load_model[model_param_name]})
                    elif fnmatch(model_param_name, param_name_action):
                        if self.config.AttentionConcat and self.config.nAttentionHeads > 1:
                            pass
                        else:
                            print( "--- Loaded weight from pretrained trained layers -- \n\t{}\n".format(model_param_name))
                            init_model.update({model_param_name: load_model[model_param_name]})
                ...
                for model_param_name, model_param in self.model.named_parameters():
                    if "onv" in model_param_name:
                        model_param.requires_grad = False
                    else:
                        model_param.requires_grad = True
                    print(model_param_name, model_param.requires_grad)
                self.start_with_freeze_SomeLayer = True

                # for model_param_name, model_param_value in self.model.named_parameters():
                #
                #     if any(fnmatch(model_param_name, load_param_name) for load_param_name in list_loaded_param_name):
                #         print("--- (Modelled) Loaded weight from pretrained trained layers -- \n\t{}\n".format(model_param_name))#, model_param_value))

                del self.optimizer
                del self.scheduler
                # define optimizers
                self.optimizer = optim.Adam(filter(lambda p:p.requires_grad, self.model.parameters()), lr=self.config.learning_rate, weight_decay=self.config.weight_decay)
                print(self.config.weight_decay)
                self.scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=self.config.max_epoch, eta_min=1e-6)
    ...
    def reset_trainability(self):
        self.optimizer.add_param_group({'params': filter(lambda p: p.requires_grad == False, self.model.parameters())})
        for model_param_name, model_param in self.model.named_parameters():
            if model_param.requires_grad == False:
                model_param.requires_grad = True
            print(model_param_name, model_param.requires_grad)
        # del self.optimizer
        # del self.scheduler
    ...
    def train(self):
        ...
        # for epoch in range(1, self.config.max_epoch + 1):
            ...
            if self.config.num_agents >= 10:
                ...
                if epoch >=25 and self.start_with_freeze_SomeLayer:
                    self.reset_trainability()
                    self.start_with_freeze_SomeLayer = False
