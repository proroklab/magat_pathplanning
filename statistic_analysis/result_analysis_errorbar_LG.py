...
class StatisticAnalysis:
    ...
    def load_data(self):
        ...
        for data_type in data.keys():
            list_a = os.walk(os.path.join(self.DATA_FOLDER, data_type))
            for subdir, dirs, files in os.walk(os.path.join(self.DATA_FOLDER, data_type)):
                ...
                for file in files:
                    ...
                    if filepath.endswith(".mat"):
                        ...
                        data[data_type].setdefault(num_agents_testing, []).append(cleaned_data)
    ...
    def collect_data(self, metrics):
        ...
        for index, label in enumerate(self.labels):
            ...
            for id_testing_agents in self.label_num_agents:
                ...
                label_HS = int(label.split('-HS')[1].split(' - TR')[0])

                id_trained_agents = int(label.split(' - TR')[1])
                # print(label_type, label_K)
                searched_results = [item for item in self.data_list
                                    if item['num_agents_trained'] == id_trained_agents
                ...
                if len(searched_results) == 0:
                    ...
                    if metrics == "mean_deltaFT":
                        ...
                        summary_label_trained_agents.extend([id_testing_agents]*size_data)
                        summary_label_data_metric.extend(list_data)

                        # load_data = searched_results[0]["list_deltaFT"]
                        #
                        # for id_case, status_reach_goal in enumerate(searched_results[0]["list_reachGoal"]):
                        #     # print(id_case,status_reach_goal)
                        #     if status_reach_goal:
                        #         summary_label_setup.append(data_label)
                        #         summary_label_trained_agents.append(id_testing_agents)
                        #         summary_label_data_metric.append(load_data[id_case])
                        #         print("add data -", load_data[id_case])
...
if __name__ == '__main__':
    ...
    labels = [
        'dcpOE - K2-HS0 - TR10', 'dcpOE - K3-HS0 - TR10',
        'dcpOE - K3-HS0 - TR20',
    ]
    title_text = 'GNN_GNN_OE'
    text_legend = [
        'GNN(OE) - K=2 - TR10', 'GNN(OE) - K=3 - TR10',
        'GNN(OE) - K=3 - TR20',
    ]
    ...
    # 1000
    DATA_FOLDER = '../MultiAgentDataset/Results_best/Statistics_generalization_LargeScale/Statistics_generalization/'
    ...
    list_num_agents = [20, 30, 40, 50, 60]
