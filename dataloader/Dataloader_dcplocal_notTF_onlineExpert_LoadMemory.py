...
class DecentralPlannerDataLoader:
    def __init__(self, config):
        ...
        if config.mode == "train":
            ...
            self.train_set = CreateDataset(self.config, "train")
            self.test_trainingSet = CreateDataset(self.config, "test_trainingSet")
            self.validStep_set = CreateDataset(self.config, "validStep")
            self.valid_set = CreateDataset(self.config, "valid")
            ...
            self.validStep_loader = DataLoader(self.validStep_set, batch_size=self.config.batch_size, shuffle=True,
                                           num_workers=self.config.data_loader_workers,
                                           pin_memory=self.config.pin_memory)

            self.test_trainingSet_loader = DataLoader(self.test_trainingSet, batch_size=self.config.valid_batch_size, shuffle=True,
                                           num_workers=self.config.data_loader_workers,
                                           pin_memory=self.config.pin_memory)
            ...
            self.test_loader = DataLoader(self.testset, batch_size=self.config.test_batch_size, shuffle=False,
                                           num_workers=self.config.data_loader_workers,
                                           pin_memory=self.config.pin_memory)
    ...
    def update_Online_Expert(self):
        self.train_set.update_Online_Expert()
        self.train_loader = DataLoader(self.train_set, batch_size=self.config.batch_size, shuffle=True,
                                       num_workers=self.config.data_loader_workers,
                                       pin_memory=self.config.pin_memory)

    def update_validset(self):
        self.valid_set = CreateDataset(self.config, "valid")
        self.valid_loader = DataLoader(self.valid_set, batch_size=self.config.valid_batch_size, shuffle=False,
                                       num_workers=self.config.data_loader_workers,
                                       pin_memory=self.config.pin_memory)
...
class CreateDataset(data.Dataset):
    def __init__(self, config, mode):
        ...
        if mode == "train":
            ...
            num_loaded = 0
            total_cases = len(set(self.data_paths))
            for i in range(len(self.data_paths)):
                if self.data_paths[i] not in self.data_cache.keys():
                    data_contents = sio.loadmat(self.data_paths[i])
                    self.data_cache[self.data_paths[i]] = data_contents
                    num_loaded += 1
                    print('Loading train data {}/{}'.format(num_loaded, total_cases), end='\r')
            ...
            for i in range(len(self.data_paths)):
                if self.data_paths[i] not in self.data_cache.keys():
                    ...
                    self.data_cache[self.data_paths[i]] = data_contents
                    num_loaded += 1
                    print('Loading test_training data {}/{}'.format(num_loaded, total_cases), end='\r')
            ...
            for i in range(len(self.data_paths)):
                if self.data_paths[i] not in self.data_cache.keys():
                    ...
                    self.data_cache[self.data_paths[i]] = data_contents
                    num_loaded += 1
                    print('Loading valid data {}/{}'.format(num_loaded, total_cases), end='\r')
            ...
            for i in range(len(self.data_paths)):
                if self.data_paths[i] not in self.data_cache.keys():
                    ...
                    self.data_cache[self.data_paths[i]] = data_contents
                    num_loaded += 1
                    print('Loading test data {}/{}'.format(num_loaded, total_cases), end='\r')

        self.data_size = len(self.data_paths)
        print('finish loading {} data...'.format(mode), num_loaded, "\n")
    ...
    def update_Online_Expert(self):
        ...
        for i in range(len(self.data_paths)):
            if self.data_paths[i] not in self.data_cache.keys():
                ...
                self.data_cache[self.data_paths[i]] = data_contents
                num_loaded += 1
                print('Loading Online Expert data', num_loaded)
        self.data_size = len(self.data_paths)
        print('Updated dataset size:', self.data_size)
    ...
    def load_train_data(self, path, id_step):
        ...
        data_contents = self.data_cache[path]
